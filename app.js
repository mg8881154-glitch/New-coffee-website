...
function starsHTML(r){
  return "★".repeat(Math.floor(r))+(r%1>=0.5?"half":"")+"☆".repeat(5-Math.floor(r)-(r%1>=0.5?1:0));
}
  ...
  grid.innerHTML=list.map(p=>`
    <div class="product-card" onclick="openDetail(${p.id})">
      ...
      <div class="card-body">
        ...
        <div class="card-rating"><span class="stars">${"★".repeat(Math.floor(p.rating))}${"☆".repeat(5-Math.floor(p.rating))}</span><span class="rating-num">${p.rating}</span></div>
...
function updateCartCount(){
  const t=Object.values(cart).reduce((a,b)=>a+b,0);
  ["cartCount","cartCountM"].forEach(id=>{const el=document.getElementById(id);if(el)el.innerText=t;});
}
...
function openDetail(id){
  ...
  document.getElementById("pmStars").innerHTML="★".repeat(Math.floor(p.rating))+"☆".repeat(5-Math.floor(p.rating));
...
function changeModalQty(d){const el=document.getElementById("pmQty");el.innerText=Math.max(1,parseInt(el.innerText)+d);}
...
function toggleWish(id){
  ...
  ["wishlistCount","wishlistCountM"].forEach(i=>{const el=document.getElementById(i);if(el)el.innerText=wishlist.size;});
  filterProducts();
...
function openWishlistModal(){
  ...
  else c.innerHTML=[...wishlist].map(id=>{const p=products.find(x=>x.id==id);return`<div class="cart-item"><img src="${p.img}?w=120" alt="${p.name}"><div class="cart-item-info"><div class="cart-item-name">${p.name}</div><div class="cart-item-price">$${p.price.toFixed(2)}</div></div><button class="cart-item-remove" onclick="toggleWish(${id});openWishlistModal()">X</button></div>`;}).join("");
  openModal("wishlistModal");
}

function openCartModal(){
  renderCart();
  const ci=document.getElementById("couponInput");const cm=document.getElementById("couponMsg");
...
function renderCart(){
  ...
  if(!keys.length){if(c)c.innerHTML='<div class="empty-state"><div class="empty-icon">🛒</div><p>Cart is empty</p></div>';["cartSubtotal","cartTotal"].forEach(id=>{const el=document.getElementById(id);if(el)el.innerText="";});return;}
  let sub=0;
  if(c)c.innerHTML=keys.map(id=>{const p=products.find(x=>x.id==id);const s=p.price*cart[id];sub+=s;return`<div class="cart-item"><img src="${p.img}?w=120" alt="${p.name}"><div class="cart-item-info"><div class="cart-item-name">${p.name}</div><div class="cart-item-price">${cart[id]} x $${p.price.toFixed(2)}</div></div><span style="color:var(--gold);font-weight:600;">$${s.toFixed(2)}</span><button class="cart-item-remove" onclick="removeFromCart(${id})">X</button></div>`;}).join("");
...
function applyCoupon(){
  ...
  if(COUPONS[code]){discount=COUPONS[code];msg.style.color="#6dbf6d";msg.innerText="Coupon applied! "+discount+"% off";renderCart();}
  else{discount=0;msg.style.color="#e05555";msg.innerText="Invalid coupon code";}
}

function proceedToOrder(){closeAllModals();const el=document.getElementById("contact");if(el)el.scrollIntoView({behavior:"smooth"});}
...
function updateOrderSummary(){
  ...
  items.innerHTML=keys.map(id=>{const p=products.find(x=>x.id==id);const s=p.price*cart[id];sub+=s;return`<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid rgba(201,168,76,0.08);font-size:0.88rem;"><span style="color:var(--cream)">${p.name} x ${cart[id]}</span><span style="color:var(--gold)">$${s.toFixed(2)}</span></div>`;}).join("");
  const total=sub*(1-discount/100);
...
function openOrdersModal(){
  ...
  else c.innerHTML=history.map(o=>`<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(201,168,76,0.1);border-radius:10px;padding:16px;margin-bottom:12px;"><div style="font-size:0.75rem;color:var(--text-muted);margin-bottom:8px;">📅 ${o.date}</div><div style="font-weight:600;color:var(--cream);margin-bottom:4px;">${o.name}</div><div style="font-size:0.82rem;color:var(--text-muted);margin-bottom:4px;">${o.items.join(", ")}</div><div style="color:var(--gold);font-weight:600;margin-top:8px;">Total: $${o.total}</div></div>`).join("");
  openModal("ordersModal");
}

function openModal(id){
  const bd=document.getElementById("modalBackdrop");const el=document.getElementById(id);
  if(bd)bd.style.display="block";if(el)el.style.display="flex";
}
function closeAllModals(){
  const bd=document.getElementById("modalBackdrop");if(bd)bd.style.display="none";
  ["cartModal","productModal","wishlistModal","ordersModal","authModal","profileModal"].forEach(id=>{const el=document.getElementById(id);if(el)el.style.display="none";});
}
...
function handleAuthClick(){
  if(currentUser){const pn=document.getElementById("profileName");const pe=document.getElementById("profileEmail");if(pn)pn.innerText=currentUser.name;if(pe)pe.innerText=currentUser.email;openModal("profileModal");}
...
function switchAuthTab(tab){
  ...
  const at=document.getElementById("authModalTitle");const tl=document.getElementById("tabLogin");const ts=document.getElementById("tabSignup");
  if(lf)lf.style.display=isLogin?"flex":"none";if(sf)sf.style.display=isLogin?"none":"flex";
...
function logoutUser(){currentUser=null;localStorage.removeItem("acUser");updateAuthUI();closeAllModals();showToast("Logged out.");}
function togglePass(inputId,btn){const inp=document.getElementById(inputId);if(inp.type==="password"){inp.type="text";btn.innerText="🙈";}else{inp.type="password";btn.innerText="👁";}}
...
function closeMobileMenu(){
  const hb=document.getElementById("hamburger");if(hb)hb.classList.remove("open");
